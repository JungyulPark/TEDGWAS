...
We considered 4,462 druggable genes [16]. Eligible instruments were genetic variants within 1 Mb of a gene that were associated with its blood expression at *P* < 5×10⁻⁸. We selected approximately independent variants by linkage-disequilibrium clumping in PLINK [19] against the 1000 Genomes Phase 3 European reference panel [18], which matched the ancestry of the exposure data. This left 6,135 instruments for 2,544 genes. The minimum instrument-strength F statistic was 29.7. Alleles were aligned between expression and disease datasets; ambiguous or unavailable variants were excluded. UKB estimates were converted from their reported linear-model scale to log-odds before analysis. Exposure effect sizes were reconstructed from the eQTLGen Z statistics, per-variant sample sizes and reference-panel allele frequencies [17]; the formulas, selection parameters and exclusions are provided in Supplementary Methods.

We used the Wald ratio for genes with one instrument and inverse-variance weighting for genes with multiple instruments [20]. Results are reported as odds ratios (ORs), 95% confidence intervals (CIs) and two-sided *P* values. ORs are per unit increase in reconstructed, genetically proxied blood expression. The reconstruction sets the variance of expression to one, so this unit is approximately one standard deviation of genetically proxied expression rather than any measured quantity; the estimates are not per drug dose and should not be interpreted as expected treatment effects. Discovery significance required *P* < 1.965×10⁻⁵, correcting for 2,544 eligible genes. Associations at *P* < 0.05 in the additional outcomes were considered nominal support only when their direction agreed with discovery. We report all estimates for the three selected genes, including non-significant results. Alternative MR estimators and tests for inconsistent instrument effects are presented in Table S1 [21, 22].
...
Expression and disease showed strong shared-variant support in BBJ and FinnGen, with probabilities of 0.951 and 0.986, respectively. The leading variant was rs179252 in both. UKB did not show the same support (0.226), despite a strong MR association. There the posterior favoured distinct causal variants (PP.H3 = 0.774), so the expression and disease signals were more consistent with two linked variants than with one shared variant. Thus, a highly significant MR result alone did not establish a shared genetic signal in every outcome.
...
19. Chang CC, Chow CC, Tellier LC, Vattikuti S, Purcell SM, Lee JJ. Second-generation PLINK: rising to the challenge of larger and richer datasets. Gigascience. 2015;4:7. doi:10.1186/s13742-015-0047-8.

# =============================================================================
# TED-TRAP — Supplementary Figure S1 FINAL polish
# Fix 1: Subtitle wrapping to prevent truncation
# Fix 2: Gold border z-order over column separator
# =============================================================================
...
long <- mat %>%
    transmute(
        ...
        `MR` = case_when(
            is.na(mr_pval) ~ NA_real_, mr_pval < 1e-10 ~ 1.0,
            mr_pval < 0.001 ~ 0.85, mr_pval < 0.05 ~ 0.55, TRUE ~ 0.15
        ),
        `Coloc` = case_when(
            is.na(coloc_PPH4) ~ NA_real_, coloc_PPH4 > 0.9 ~ 0.95,
            coloc_PPH4 > 0.8 ~ 0.75, TRUE ~ 0.3
        ),
        `MVMR` = case_when(is.na(mvmr_flag) ~ NA_real_, TRUE ~ 0.8),
        `Tissue` = case_when(
            is.na(tis_padj) ~ NA_real_, tis_padj < 0.01 ~ 0.9,
            tis_padj < 0.05 ~ 0.6, tis_padj < 0.1 ~ 0.3, TRUE ~ 0.1
        ),
        `Conv.` = conv_score / 4
    ) %>%
    pivot_longer(
...
# =============================================================================
# PATCH: Subtitle wrapped to 2 lines
# =============================================================================
subtitle_text <- paste0(
    "Colocalization and MVMR were performed only on genes meeting the primary MR significance threshold (P < 0.05).\n",
    "Grey cells denote conditional non-testing rather than null results."
)

# =============================================================================
# Build plot with PATCHED layer order
# =============================================================================
suppS1 <- ggplot(long, aes(x = evidence, y = gene, fill = score)) +
    ...
    # Layer 3: column separator (BEFORE gold border)
    geom_vline(xintercept = 4.5, linewidth = 0.7, color = "grey25") +
    # Layer 4: group dashed separators
    geom_hline(
        yintercept = c(6.5, 11.5, 14.5), linewidth = 0.35,
        color = "grey35", linetype = "22"
    ) +
    # Layer 5: PATCH — Gold border LAST (on top of everything)
    annotate("rect",
...
# =============================================================================
# Save — PATCH: width expanded 9.5 → 11 for subtitle full display
# =============================================================================
ggsave("TrackA_MR/figures/FigureS1_stepwise_screening.pdf", suppS1,
    width = 11, height = 11, device = cairo_pdf, bg = "white"
)
...
ggsave("TrackA_MR/figures/FigureS1_stepwise_screening.tiff", suppS1,
    width = 11, height = 11, dpi = 600, compression = "lzw", bg = "white"
)

cat("\n✅ Supp S1 FINAL polish saved\n")
cat("   - Subtitle wrapped to 2 lines (no truncation)\n")
cat("   - Gold border z-order fixed (drawn on top of column separator)\n")
cat("   - Figure width: 9.5 → 11 inches\n")

# =============================================================================
# TED-TRAP — Supp S1 FINAL FINAL (v3 MVMR ground truth)
# Source change: 04_mvmr_summary.csv → 04v3_mvmr_finngen_summary.csv
# =============================================================================
...
mvmr <- read.csv("TrackA_MR/results/04v3_mvmr_finngen_summary.csv", stringsAsFactors = FALSE) # PATCH
tis <- read.csv("TrackA_MR/results/06b_candidate_deseq2_reanalysis.csv",
    stringsAsFactors = FALSE
)

cat("=== v3 MVMR (ground truth) ===\n")
print(mvmr)
...
# PATCH: v3에서 TSHR P-value 추출 (column 이름 재확인 필요)
# 우선 가능한 column 이름들 다 체크
cat("\nv3 column names:", paste(names(mvmr), collapse = ", "), "\n")

# TSHR row 찾기
mvmr_tshr_row <- mvmr[grepl("TSHR", mvmr$Analysis, ignore.case = TRUE), ]
if (nrow(mvmr_tshr_row) == 0 && "Exposure" %in% names(mvmr)) {
    mvmr_tshr_row <- mvmr[grepl("TSHR", mvmr$Exposure, ignore.case = TRUE), ]
}

cat("\nTSHR row in v3:\n")
print(mvmr_tshr_row)

# P-value column 자동 탐지
pcol <- intersect(c("P_adjusted", "P_value", "pval", "P", "P_adj"), names(mvmr))[1]
bcol <- intersect(c("Beta_adjusted", "beta", "B", "Beta"), names(mvmr))[1]
cat("Using P column:", pcol, "| Beta column:", bcol, "\n")

mvmr_tshr_p <- mvmr_tshr_row[[pcol]]
mvmr_tshr_b <- mvmr_tshr_row[[bcol]]
cat("TSHR MVMR (v3):  β =", mvmr_tshr_b, "| P =", mvmr_tshr_p, "\n")
...
cat("\nTSHR triangulation score:", mat$triang_score[mat$gene == "TSHR"], "/ 4\n")
...
long <- long %>%
    ...
    mutate(label = case_when(
        ...
        evidence == "MVMR (P)" & !is.na(mvmr_pval) ~ sprintf("%.3f", mvmr_pval), # PATCH: 0.657 표시
...
# ---- Subtitle (PATCH: outcome 명시) ----
subtitle_text <- paste0(
    "Colocalization and MVMR were performed only on genes meeting the primary MR significance threshold (P < 0.05).\n",
    "MVMR outcome: Graves ophthalmopathy (FinnGen R12). Grey cells denote conditional non-testing rather than null results."
)

# =============================================================================
# Build plot (structure identical to previous, only data source changed)
# =============================================================================
suppS1 <- ggplot(long, aes(x = evidence, y = gene, fill = score)) +
    ...
    theme(
        ...
        plot.margin = margin(5, 60, 5, 120)
    )

# KEEPING THE -0.3 margin modification we identified earlier!
suppS1 <- suppS1 +
    annotation_custom(
...
cat("\n✅ Supp S1 regenerated with v3 MVMR (Graves ophthalmopathy outcome)\n")
cat("   TSHR MVMR P: 0.912 (old, wrong) → 0.657 (v3, correct)\n")
cat("   TSHR Triangulation: 3/4 (unchanged — both null)\n")

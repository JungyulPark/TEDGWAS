...
panelA <- ggplot(tshr_sample, aes(x = label, y = tpm, fill = group)) +
    ...
    annotate("text",
        x = 5.2, y = ted_mean, label = "TED mean",
        size = 2.9, color = "#8B0000", hjust = 0, fontface = "italic"
    ) +
...
ggsave("TrackA_MR/figures/Figure3_main_3panel_v4.pdf", combined,
    width = 11, height = 8, device = cairo_pdf, bg = "white"
) # PATCH: height 9→8
ggsave("TrackA_MR/figures/Figure3_main_3panel_v4.png", combined,
    width = 11, height = 8, dpi = 600, bg = "white"
)
ggsave("TrackA_MR/figures/Figure3_main_3panel_v4.tiff", combined,
    width = 11, height = 8, dpi = 600, compression = "lzw", bg = "white"
)

cat("\n✅ Figure 3 Main v4 saved (TED mean unclipped + Instrument-validity anchor)\n")

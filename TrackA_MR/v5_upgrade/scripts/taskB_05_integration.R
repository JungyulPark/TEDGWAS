...
build_signal_row <- function(snp, idx) {
  ...
  data.frame(
    ...
    ld_with_rs179252_eur = if (nrow(c_eur) > 0) c_eur$ld_with_rs179252_eur[1] else if (nrow(s_eur) > 0) s_eur$ld_top_with_rs179252_eur[1] else NA_real_,
    ld_with_rs179252_eas = if (nrow(c_eas) > 0) c_eas$ld_with_rs179252_eas[1] else if (nrow(s_eas) > 0) s_eas$ld_top_with_rs179252_eas[1] else NA_real_,
...
combined$usable_as_independent_iv <- with(combined, {
  ...
  cond_p_ok[is.na(cond_p_ok)] <- FALSE
  
  ld_low <- pmax(ld_with_rs179252_eur, ld_with_rs179252_eas, na.rm = TRUE) < 0.1
  ld_low[is.na(ld_low)] <- FALSE
  
  pip_high <- pmax(ifelse(is.na(susie_pip_eur), 0, susie_pip_eur),
                   ifelse(is.na(susie_pip_eas), 0, susie_pip_eas)) > 0.5
  ...
  is_rs179252 <- (lead_snp_cojo_eur == "rs179252" |
                  ...
                  lead_snp_cojo_eas == "rs179252" |
                  lead_snp_susie_eas == "rs179252")
  is_rs179252[is.na(is_rs179252)] <- FALSE

  res <- cond_p_ok & ld_low & pip_high
  res[is.na(res)] <- FALSE
  ifelse(is_rs179252, FALSE, res)
})

combined$notes <- with(combined, ifelse(
  usable_as_independent_iv, "Usable as independent IV for multi-IV MR",

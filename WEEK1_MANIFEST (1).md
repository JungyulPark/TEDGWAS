...
## 5. OUTSTANDING ITEMS (resolved 2026-05-22)

| Item | Status | Action |
|------|--------|--------|
| TaskB_00_ld_reference_metadata.csv exists? | CONFIRMED EXISTS (729 bytes) — content pending Claude verification | Upload for content check |
| TaskB_02_ld_clumping_sensitivity.csv exists? | CONFIRMED EXISTS (1,296 bytes) — content pending Claude verification | Upload for content check |
...
- **Supplementary Table** (later): TSHR fine-mapping (COJO + SuSiE independent-signal analysis) — evidence for single causal signal. Source: WEEK1_RESULTS_consolidated.csv
- **Supplementary Table** (later): rs179252 allele-direction audit — directionality paradox transparency. Source: WEEK1_RESULTS_consolidated.csv (ALLELE_AUDIT rows)
- **Supplementary Table** (later): LD reference metadata. Source: TaskB_00
- **Supplementary Figure S1** (later): TSHR locus regional association plot
  - x-axis: chr14 TSHR ±1Mb; y-axis: −log10 P for TSHR cis-eQTL
  - rs179252 highlighted; COJO-selected SNPs + SuSiE credible sets marked; TSHR gene track
  - Message: TSHR locus is an rs179252-centered single-signal structure
- **Supplementary Figure S2** (later): directionality interpretation schematic
  - rs179252 G allele → blood TSHR ↑ → GD MR protective; ≠ TED orbital tissue TSHR ↑
  - CRITICAL: must NOT visually imply "protective = good biology." Core caption: "Blood cis-eQTL direction should not be interpreted as orbital tissue expression direction."

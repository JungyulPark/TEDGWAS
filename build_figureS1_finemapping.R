...
pA <- ggplot(cs, aes(pos_kb, panel, color=panel)) +
  geom_segment(aes(x=min(pos_kb)-2, xend=max(pos_kb)+2, y=panel, yend=panel),
               color="grey85", linewidth=0.6) +
  geom_point(aes(size=is_primary, shape=is_primary)) +
  ggrepel::geom_text_repel(aes(label=snp), size=3, direction="y",
                           box.padding=0.5, segment.color="grey70", show.legend=FALSE) +
  ...
  labs(title="A  Credible-set lead SNP positions (TSHR locus, chr14)",
       x="Position relative to 81.4 Mb (kb, hg19)", y=NULL) +

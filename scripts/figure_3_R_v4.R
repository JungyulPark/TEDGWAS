# ============================================================
# Figure 3 v4 — Three-panel composite (TED-TRAP v4.5)
# v4 changes:
#  - Panel B cohort label: "In-house RNA-seq (n=5)" 
#    → "In-house orbital adipose (n=5)†"
#  - Panel B subtitle: footnote about TED orbital + nasal control
#  - All other panels unchanged from v3
# ============================================================
...
# ============================================================
# PANEL A — TSHR per-sample TPM (UNCHANGED from v3)
# ============================================================
...
# ============================================================
# PANEL B — Insulin cassette 3-cohort (★ v4 LABEL CORRECTION)
# ============================================================

panel_b_data <- data.frame(
  Gene = factor(rep(c("INSR", "IRS2", "FOXO1", "PIK3R1", "PDPK1"), 3),
                levels = c("INSR", "IRS2", "FOXO1", "PIK3R1", "PDPK1")),
  # ★ v4 NEW LABEL: In-house orbital adipose (with compartment mismatch footnote)
...
# ★ v4 NEW SUBTITLE: compartment mismatch footnote
panel_b_subtitle <- paste0(
  "\u2020 In-house = 4 TED orbital adipose vs 1 medial nasal fat control ",
  "(compartment mismatch acknowledged).\n",
  "In-house 5/5 upregulation NOT reproduced in matching-compartment GSE58331; ",
  "INSR, IRS2, PIK3R1 directionally opposite.\n",
...
# ============================================================
# PANEL C — IGF1R cross-phenotype MR (UNCHANGED from v3)
# ============================================================
...
p_c <- ggplot(panel_c_data, aes(x = beta, y = outcome, color = category)) +
  ...
  annotate("text", x = 0.20, y = "Height (IGF-axis pathway anchor)",
           ...
           vjust = -1.2, color = "#BF9000", size = 3.4,
           fontface = "italic") +
...
# ============================================================
# Assemble + 600 DPI but smaller dimensions to stay under 40 MP
# ============================================================

top_row    <- p_a + p_b + plot_layout(widths = c(1, 1.5))
final_plot <- top_row / p_c + plot_layout(heights = c(1.1, 0.85))

# Use 12 x 8 inches to ensure under 40 MP at 600 DPI
# 12 * 600 = 7200; 8 * 600 = 4800 → 34.6 MP ✅
ggsave(
...
cat("Figure 3 v4 saved.\n")
cat("Panel B label: 'In-house orbital adipose (n=5)†'\n")
cat("Panel B footnote: '† 4 TED orbital adipose vs 1 medial nasal fat control'\n")
cat("Dimensions: 12x8 in @ 600 DPI = 7200 x 4800 px = 34.6 MP (under 40 MP limit)\n")

# ============================================================
# Figure 3 v6.1 — Three-panel vertical stack (TED-TRAP v4.5)
# v6.1 changes (FROM v6):
#  - Panel A: y-axis 범위 1.15 → 1.05 (막대 위 빈 공간 축소)
#  - Panel A: bar width 0.55 → 0.7 (더 두꺼운 막대로 가독성 향상)
#  → "TSHR expression (TPM)" 축 라벨이 막대 차트와 가깝게
# ============================================================
...
p_a <- ggplot(panel_a_data, aes(x = sample, y = TPM, fill = group)) +
  ...
  annotate("text", x = 4.7, y = ted_mean + 0.06,
           label = sprintf("TED mean = %.2f", ted_mean),
           color = "darkred", size = 3.5, fontface = "italic", hjust = 1) +
  ...
  labs(title    = "(A) TSHR per-sample expression in orbital tissue",
       subtitle = "Exploratory in-house signal: log2FC = +2.33, padj = 0.006   |   TED orbital adipose vs nasal fat control",
       x = "Sample",
...
# 9 x 14 inches @ 500 DPI = 4500 x 7000 = 31.5 MP < 40 MP ✅
ggsave(
...
cat("Figure 3 v6 saved.\n")
cat("Layout: VERTICAL STACK (A / B / C)\n")
cat("- Panel A: full width, annotation moved to subtitle\n")
cat("- Panel B: full width, subtitle on single line\n")
cat("- Panel C: full width\n")
cat("Dimensions: 9 x 14 in @ 500 DPI = 4500 x 7000 px = 31.5 MP\n")
cat("All titles/subtitles LEFT-aligned (hjust=0)\n")

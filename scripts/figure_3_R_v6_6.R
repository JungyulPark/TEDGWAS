# ============================================================
# Figure 3 v6.6 — Three-panel vertical stack (TED-TRAP v4.5)
# v6.6 핵심 fix: patchwork의 axis alignment 문제 해결
#
# 진짜 원인:
# - Panel C가 forest plot이라 y-axis text가 매우 김
#   ("Height (IGF-axis pathway anchor)", "Type 2 diabetes (T2D)" 등)
# - patchwork vertical stack에서 모든 panel을 axis align
# - 결과: Panel A, B가 Panel C의 긴 y-axis text 폭만큼 좌측 padding
# - → Panel A, B 좌측에 거대한 빈 공간 발생
#
# 해결: plot_layout(axes = "collect_y") 미사용 + 
#       wrap_plots() 사용 (각 panel 독립 정렬)
#       또는 Panel C의 y-axis text를 짧게
# ============================================================
...
# ★ v6.6: y-axis labels 짧게 → patchwork align 시 좌측 빈공간 ↓
# ============================================================
...
panel_c_data <- data.frame(
  ...
  outcome = factor(c("Fasting glucose",
                     "HbA1c",
                     "T2D",                           # was: "Type 2 diabetes (T2D)"
                     "Height\u2021"),                 # was: "Height (IGF-axis pathway anchor)"
                   levels = c("Height\u2021",
                              "T2D",
                              "HbA1c",
                              "Fasting glucose")),
...
p_c <- ggplot(panel_c_data, aes(x = beta, y = outcome, color = category)) +
  ...
  labs(title    = "(C) IGF1R cross-phenotype Mendelian randomization",
       subtitle = "Germline IGF1R does not cause metabolic phenotypes (\u2021 = IGF-axis pathway anchor)",
...
# ============================================================
# Vertical stack — using wrap_plots() for independent alignment
# ============================================================

# ★ v6.6: wrap_plots() with align = "v" only on x-axis
# This aligns x-axes but NOT y-axes → each panel独립적 좌측 정렬
final_plot <- wrap_plots(p_a, p_b, p_c, ncol = 1,
                         heights = c(1.0, 1.3, 1.0)) +
  plot_annotation(
...
cat("Figure 3 v6.6 saved.\n")
cat("핵심 fix:\n")
cat("  ✅ Panel C y-axis labels 짧게 (Height‡, T2D)\n")
cat("  ✅ wrap_plots() 사용 — 각 panel 독립적 axis 정렬\n")
cat("  ✅ patchwork의 axis alignment forced collect 회피\n")
cat("  ✅ 좌측 빈 공간 제거\n")
cat("Dimensions: 11 x 14 in @ 470 DPI = 5170 x 6580 px = 34.0 MP\n")

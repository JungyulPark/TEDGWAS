# ============================================================
# Figure 3 v7 — Three-panel vertical stack (TED-TRAP v4.5)
# v7 핵심 fix (선생님 제안):
#  1. Panel C y-axis labels: a/b/c/d (1글자) → axis padding 최소화
#  2. plot.title.position 제거 → title이 차트와 일관된 정렬 (panel 기준)
#  3. 모든 plot.subtitle margin 단축 → 차트와 가깝게
#  4. 결과: patchwork align 영향 최소화 → Panel A 좌측 빈공간 자동 해결
# ============================================================
...
# Subtitle에 a/b/c/d 정의 명시
panel_c_subtitle <- paste0(
  "Outcomes: a = Fasting glucose, b = HbA1c, c = Type 2 diabetes (T2D), ",
  "d = Height (IGF-axis pathway anchor)\n",
  "Germline IGF1R does not cause metabolic phenotypes (null IVW); ",
  "height retains pathway-level activity"
)
...
p_c <- ggplot(panel_c_data, aes(x = beta, y = outcome, color = category)) +
  ...
  theme(plot.title    = element_text(face = "bold", size = 13, hjust = 0,
        ...
        plot.subtitle = element_text(face = "italic", size = 10,
                                     ...
                                     margin = margin(b = 4),
                                     lineheight = 1.2),
        # ★ v7: y-axis labels이 a/b/c/d (1글자) → bold + 큰 size
...
cat("Figure 3 v7 saved.\n")
cat("핵심 fix (선생님 제안):\n")
cat("  ✅ Panel C y-axis labels: a/b/c/d (1글자) — axis padding 최소화\n")
cat("  ✅ plot.title.position 제거 → title이 차트와 일관된 정렬\n")
cat("  ✅ subtitle margin 축소 → 차트와 가깝게 (b=8 → b=4)\n")
cat("  ✅ plot.margin 좌측 5pt → 빈공간 최소화\n")

# ============================================================
# Figure 3 v7.3 — Three-panel vertical stack (TED-TRAP v4.5)
# v7.3 변경 (FROM v7.2):
#  - a/b/c/d 정의를 plot CAPTION으로 (plot 하단)
#  - 데이터(error bars)와 겹침 없음
#  - 표준 figure 관례 (footnote 형태)
# ============================================================
...
p_c <- ggplot(panel_c_data, aes(x = beta, y = outcome, color = category)) +
  ...
  labs(title    = "(C) IGF1R cross-phenotype Mendelian randomization",
       ...
       y        = NULL,
       caption  = "a = Fasting glucose | b = HbA1c | c = Type 2 diabetes (T2D) | d = Height (IGF-axis pathway anchor)",
  ...
  theme(plot.title    = element_text(face = "bold", size = 13, hjust = 0,
        ...
        legend.key.size    = unit(0.4, "cm"),
        legend.box.margin  = margin(t = 0, b = 3),
        # ★ v7.3: caption styling — plot 하단 좌측, 검정색
        plot.caption       = element_text(size = 9.5, color = "black",
                                          hjust = 0,
                                          margin = margin(t = 6),
                                          face = "plain"),
...
cat("Figure 3 v7.3 saved.\n")
cat("핵심 변경 (FROM v7.2):\n")
cat("  ✅ a/b/c/d 정의를 plot CAPTION으로 (panel 하단)\n")
cat("  ✅ 데이터와 겹침 없음 — 표준 footnote 관례\n")

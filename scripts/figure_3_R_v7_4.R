# ============================================================
# Figure 3 v7.4 — Three-panel vertical stack (TED-TRAP v4.5)
# v7.4 변경 (FROM v7.3):
#  - a/b/c/d 정의를 plot 내부 INSET BOX (좌측 상단, 'a' row 기준)
#  - Caption 제거
#  - 검정색 plain text + white background + thin border
#  - 데이터(error bars)와 겹치지 않게 위치 조정
# ============================================================
...
p_c <- ggplot(panel_c_data, aes(x = beta, y = outcome, color = category)) +
  ...
  annotate("text", x = 0.20, y = "d",
           ...
           fontface = "italic") +
  # ★ v7.4: a/b/c/d 정의 inset box ('a' row 위쪽 좌측)
  ...
  # ★ v7.4: y limits 확장하여 inset box 위쪽 공간 확보
...
cat("Figure 3 v7.4 saved.\n")
cat("핵심 변경 (FROM v7.3):\n")
cat("  ✅ a/b/c/d 정의를 plot 내부 INSET BOX (좌측 상단, 'a' row 기준)\n")
cat("  ✅ 검정색 + white background + thin border (사각형)\n")

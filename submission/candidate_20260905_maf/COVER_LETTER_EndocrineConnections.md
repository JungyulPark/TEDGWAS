...
Please consider our manuscript, “Genetic susceptibility at TSHR and IGF1R in Graves disease and thyroid eye disease,” as an original research article.
...
The manuscript reports limitations that affect clinical interpretation: FinnGen uses population controls and cannot isolate TED-specific liability within Graves disease; the exposure data are European blood eQTLs; colocalization depends on the single-causal-variant model and prior; and reconstructed effect magnitudes depend on exposure frequencies. Cohort-specific frequency sensitivity maintained the discovery-hit set and colocalization classifications; its MR and detection-threshold changes are reported. MR detection thresholds are reported separately from the unquantified power of the combined filter. Orbital RNA-seq observations from four TED samples and one control are descriptive only, with no inferential expression tests reported.

...
The manuscript reports limitations that affect clinical interpretation: FinnGen uses population controls and cannot isolate TED-specific liability within Graves disease; the exposure data are European blood eQTLs; colocalization depends on the single-causal-variant model and prior; and cohort-specific eQTLGen frequency sensitivity preserved the discovery-hit set and colocalization classifications while changing some MR magnitudes and detection thresholds. MR detection thresholds are reported separately from the unquantified power of the combined filter. Orbital RNA-seq observations from four TED samples and one control are descriptive only, with no inferential expression tests reported.
...
for name,txt in texts.items():
    ...
    for p in d.paragraphs:
        ...
        if p.text=='Tables' or (issup and p.text=='Supplementary Figures'):p._element.getparent().remove(p._element)
        if iscover:
            p.paragraph_format.widow_control=True

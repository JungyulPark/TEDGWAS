...
# Figure S1 requires authorized local per-sample points, excluded from this package.
import argparse
parser=argparse.ArgumentParser()
parser.add_argument('--tissue-points',type=Path,help='Authorized local CSV with gene, ctrl_tpm, ted_tpm, ted_per_sample, log2fc_tpm')
args=parser.parse_args()
ti=pd.read_csv(args.tissue_points) if args.tissue_points else None
if ti is not None:
    summary=pd.read_csv(P/'tissue_descriptive.csv').set_index('gene')
    points=ti.set_index('gene')
    assert points.index.is_unique and set(points.index)==set(summary.index)
    for gene,row in points.iterrows():
        values=np.array([float(v) for v in row.ted_per_sample.split(';')])
        assert len(values)==4 and np.isclose(values.mean(),row.ted_tpm,atol=1e-8)
        for field in ['ctrl_tpm','ted_tpm','log2fc_tpm']:
            assert np.isclose(row[field],summary.loc[gene,field],atol=1e-8)
...
if ti is not None:
    ...
    print('Preserved Figure S1; use --tissue-points with an authorized local CSV to rebuild it.')
...
print('Built Figures 1–3/S2; Figure S1 rebuilt only when authorized local points were supplied.')

...
plt.rcParams.update({'font.family':'serif','font.serif':['Times New Roman'],'font.size':10,'pdf.fonttype':42,'axes.spines.top':False,'axes.spines.right':False})
def save(fig,name):
...
box(.19,.43,.62,.09,'13 discovery associations\nP < 0.05 / 2,544')
...
box(.56,.10,.425,.10,'0 of 6 met shared-variant support\nin both BBJ and FinnGen',10);arrow(.8425,.265,.78,.20)
ax.text(.02,.15,'Three selected genes\nTSHR  ·  IGF1R  ·  CTLA4\nCompared in every outcome\nregardless of discovery P',ha='left',va='center',fontsize=9.5)
...
ea.text(.5,12.2,'OR (95% CI)',ha='center',fontweight='bold');pa.text(.5,12.2,'P value',ha='center',fontweight='bold')
...
for g,base in zip(['TSHR','IGF1R','CTLA4'],[11,7,3]):
    ...
    for i,(oc,label) in enumerate(zip(ocs,labels)):
        ...
        pa.text(.5,y,pv(r.pvalue),ha='center',va='center',fontweight='bold',fontsize=10)

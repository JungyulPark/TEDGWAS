...
# ====================== CONFIG ======================
DRY_RUN <- TRUE   # <<< set FALSE for full run after dry-run approval
...
FRQ_FILE <- file.path(WORK_DIR, "data/1000G_EUR_phase3/g1000_eur.frq")
...
OUTCOMES <- list(
  BBJ_Graves = list(
    path = file.path(WORK_DIR, "04_druggable_mr/data/outcomes/GCST90018627_buildGRCh37.tsv.gz"),
    ancestry = "EAS", role = "discovery", n_case = 2809),
  UKB_hyperthyroid = list(
    path = file.path(WORK_DIR, "04_druggable_mr/data/outcomes/GCST90038636_buildGRCh37.tsv"),
...
load_outcome <- function(oc) {
  ...
  snp_c  <- get1(c("hm_rsid","rsid","rsids","variant_id","snp"))
